...
if [ "$HM_HMIP_DEV" == "HM-MOD-RPI-PCB" ]; then
  ...
  if [ -n "$NEW_VERSION" ] && [ "$NEW_VERSION" != "$HM_HMIP_VERSION" ]; then
    ...
    /bin/eq3configcmd update-coprocessor -p $HM_HMIP_DEVNODE -t HM-MOD-UART -u -d /firmware/HM-MOD-UART

    HM_HMIP_VERSION=`/bin/eq3configcmd update-coprocessor -p $HM_HMIP_DEVNODE -t HM-MOD-UART -c -v 2>&1 | grep "Version:" | cut -d' ' -f5`
...
if [ "$HM_HMIP_DEV" == "HMIP-RFUSB" ]; then
  ...
  if [ -n "$NEW_VERSION" ] && [ "$NEW_VERSION" != "$HM_HMIP_VERSION" ]; then
    ...
    HM_HMIP_VERSION=`JAVA_HOME=$JAVA_HOME $JAVA_HOME/bin/java $JAVA_ARGS -Dgnu.io.rxtx.SerialPorts=$HM_HMIP_DEVNODE -jar /opt/HmIP/hmip-copro-update.jar -p $HM_HMIP_DEVNODE -o -f /firmware/HmIP-RFUSB/dualcopro_update_blhmip-$NEW_VERSION.eq3 2>/dev/null | grep "Version:" | cut -d' ' -f5`
