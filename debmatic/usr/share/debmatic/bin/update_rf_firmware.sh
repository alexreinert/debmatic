...
if [ "$HM_HMRF_DEV" == "RPI-RF-MOD" ]; then
  ...
  if [ -n "$NEW_VERSION" ] && [ "$NEW_VERSION" != "$HM_HMRF_VERSION" ]; then
    ...
    HM_HMRF_VERSION=`java -Dgnu.io.rxtx.SerialPorts=$HM_HOST_GPIO_UART -jar /opt/HmIP/hmip-copro-update.jar -p $HM_HOST_GPIO_UART -o -f /firmware/RPI-RF-MOD/dualcopro_update_blhmip-$NEW_VERSION.eq3 2>/dev/null | grep "Version:" | cut -d' ' -f5`
