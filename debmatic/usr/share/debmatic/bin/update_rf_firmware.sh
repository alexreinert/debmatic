...
if [ "$HM_HMRF_DEV" == "RPI-RF-MOD" ]; then
  NEW_VERSION=$(find "/firmware/RPI-RF-MOD/dualcopro_update_blhmip-*.eq3" | sed 's/.*-\(.*\)\.eq3/\1/' | sort | tail -1)
...
if [ "$HM_HMIP_DEV" == "HMIP-RFUSB" ]; then
  NEW_VERSION=$(find "/firmware/HmIP-RFUSB/hmip_coprocessor_update-*.eq3" | sed 's/.*-\(.*\)\.eq3/\1/' | sort | tail -1)
